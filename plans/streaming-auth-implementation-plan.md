...
**`librespot/` (separate repo — Task 1 commits there, on its own branch):**
...
### Task 1: librespot-oauth — validate `state`, bound the waits
...
- Produces: `OAuthClientBuilder::new(client_id, redirect_uri, scopes).open_in_browser().build()?`
  ...
  callbacks whose `state` does not match. Task 3 calls exactly this.
...
### Task 2: Persist credentials, and init from them
...
### Task 3: The streaming grant behind the FFI
...
**Interfaces:**
- Consumes: `credentials_cache_dir()` and `create_session` from Task 2; `OAuthClientBuilder`
  from Task 1.
- Produces: `spotifly_authorize_streaming() -> i32` (0 success, -1 failure, -2 superseded), and
  `fn run_is_superseded(started_generation: u64) -> bool`. Task 7 calls the FFI through Swift.
...
### Task 4: Stop asking Swift for a token on reconnect
...
**Interfaces:**
- Consumes: `init_player_async(None, ...)` from Task 2.
...
### Task 5: `SpotifyAPI.startPlayback`
...
### Task 6: Route play to local or remote, and refresh after remote
...
**Interfaces:**
- Consumes: `SpotifyAPI.startPlayback` (Task 5).
- Produces: `enum PlaybackTarget { case local, remote(deviceId: String), needsAuthorization }`
  ...
  Task 7 shows its alert on `.needsAuthorization`.
...
### Task 7: Step 2 in the UI, and the way back to it
...
**Interfaces:**
- Consumes: `spotifly_authorize_streaming()` (Task 3), `PlaybackTarget` and
  `needsStreamingAuthorization` (Task 6).
...
contains a credentials file. Add it to `rust/src/lib.rs` alongside the Task 3 work:
...
In `LoggedInView.swift`, present the alert on the flag from Task 6:
...
### Task 8: Logout clears the streaming credentials
...
**Interfaces:**
- Consumes: `credentials_cache_dir()` (Task 2).
...
- **Task 1 is in a different repository.** It commits in `librespot/`, not `spotifly-code/`.
  The Spotifly build picks it up through the path dependency with no pin.
