    ...
    match guard.as_ref() {
        Some(result) => {
            match result.refresh_token.as_ref() {
                Some(refresh_token) => {
                    match CString::new(refresh_token.clone()) {
                        Ok(cstr) => cstr.into_raw(),
                        Err(_) => ptr::null_mut(),
                    }
                }
...
pub extern "C" fn spotifly_refresh_access_token(
    ...
    if client_id.is_null() || redirect_uri.is_null() || refresh_token.is_null() {
        eprintln!("Refresh error: client_id, redirect_uri, or refresh_token is null");
        return -1;
    }
    ...
    let refresh_token_str = unsafe {
        match CStr::from_ptr(refresh_token).to_str() {
            Ok(s) => s.to_string(),
            Err(_) => {
                eprintln!("Refresh error: invalid refresh_token string");
                return -1;
            }
        }
    };
...
async fn perform_token_refresh(
    ...
    let client = OAuthClientBuilder::new(client_id, redirect_uri, scopes).build()?;
