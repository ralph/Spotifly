...
enum SpotifyAPI {
    ...
    /// - Parameters:
    ///   - accessToken: Spotify access token
    ///   - timeRange: Time range for calculating top items
    ///   - limit: Number of artists to fetch (max 50)
    ///   - offset: Offset for pagination
    ...
    static func fetchUserTopTracks(
        ...
        let tracks = items.compactMap { item -> SearchTrack? in
            guard let id = item["id"] as? String,
                  let name = item["name"] as? String,
                  let uri = item["uri"] as? String,
                  let durationMs = item["duration_ms"] as? Int
            ...
            {
                artistName = (firstArtist["name"] as? String) ?? artistName
            ...
            if let album = item["album"] as? [String: Any] {
                albumName = (album["name"] as? String) ?? albumName
                albumId = album["id"] as? String
                if let images = album["images"] as? [[String: Any]],
                   let firstImage = images.first,
                   let urlString = firstImage["url"] as? String
            ...
            if let externalUrls = item["external_urls"] as? [String: Any] {
